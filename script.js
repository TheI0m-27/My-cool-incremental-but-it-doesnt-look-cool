...
supremeReset.onclick = function() {
    ...
    if (updateUpgrade1 === false) {
        sPupgrade.style.visibility = "visible";
    }
    if (updateUpgrade2 === false) {
...
sPupgrade2.onclick = function() {
    if (sPoints >= 1) {
        ...
        updateSPoints();
        sPupgrade2.innerText = "Current: * " + (sP * 1e6).toExponential(2);
        sPupgrade2.style.visibility = "hidden";
    }
};
sPupgrade3.onclick = function() {
    if (sPoints >= 3 && boolean3 === false) {
        sPoints -= 3;
        updateSPoints();
...
sPupgrade8.onclick = function() {
    if (sPoints >= 2000 && boolean8 === false) {
        sPoints -= 2000;
        sPmultiplier *= 1e6;
        boolean8 = true;
